...
try:
    path = str(pathlib.Path.home()) + '\\Game from Iskander' + '\\save'
    os.mkdir(path)
except:
    pass
...
def game(levels_dict):
    ...
    if gamer_input == 2 or gamer_input == 3:
        print('Select save\n')
        getFiles(path)  # вывести все файлы, содержащиеся в папке 
        while not is_continue: 
            # получить номер файла в папке
            number_file = getNumber_save(path)
            for i in files_list:
            ...
            if not is_continue:
                try:                                                # проверка на возможность чтения файла
                    game_params = reading(path, gamer_choice)
                    is_continue = True
                except:
                    print('Error, try again')
